...
const DEFAULT_STATUS_TYPES = [
	...
	{ name: 'Field/Training', color: '#a0522d', text_color: '#ffffff', sort_order: 2 },
	{ name: 'TDY', color: '#9f7aea', text_color: '#ffffff', sort_order: 3 },
	{ name: 'Appointment', color: '#ed8936', text_color: '#ffffff', sort_order: 4 },
	{ name: 'Sick', color: '#e53e3e', text_color: '#ffffff', sort_order: 5 }
];

const DEFAULT_ASSIGNMENT_TYPES = [
	{ name: 'Medical Officer of the Day', short_name: 'MOD', assign_to: 'personnel', color: '#dc2626', sort_order: 0 },
	{ name: 'Front Desk Support', short_name: 'FDS', assign_to: 'group', color: '#7c3aed', sort_order: 1 }
];
...
export const actions: Actions = {
	default: async ({ request, locals }) => {
		...
		// Use the database function to create clinic and membership atomically
		const { data: clinicId, error: clinicError } = await locals.supabase
			.rpc('create_clinic_with_owner', { p_name: name });
		...
		const { error: groupsError } = await locals.supabase.from('groups').insert(groupRows);
		...
		const { error: statusError } = await locals.supabase.from('status_types').insert(statusRows);
		...
		const { error: assignmentError } = await locals.supabase.from('assignment_types').insert(assignmentRows);
		if (assignmentError) console.error('Assignment types seed error:', assignmentError);
		...
		for (let i = 0; i < holidayRows.length; i += 50) {
			const { error: holidayError } = await locals.supabase.from('special_days').insert(holidayRows.slice(i, i + 50));
			if (holidayError) console.error('Holidays seed error:', holidayError);
		}

		throw redirect(303, `/clinic/${clinicId}`);
	}
};

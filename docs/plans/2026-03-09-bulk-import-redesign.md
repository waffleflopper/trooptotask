...
<script lang="ts">
  ...
  function setMapping(colIndex: number, fieldKey: string | null) {
    mappings = mappings.map(m =>
      m.columnIndex === colIndex ? { ...m, fieldKey } : m
    );
    onMappingChange?.(mappings);
  }
  ...
  const errorCount = $derived(
    rowValidations.filter(v => !v.valid).length
  );
...
<style>
  ...
  .cell { cursor: text; }
  .cell-error { background: var(--color-error-bg, #fde8e8); border: 1px solid var(--color-error); }
  .cell-warning { background: var(--color-warning-bg, #fff3cd); border: 1px solid var(--color-warning); }
